...
let string_of_return (str : 'v -> string) (heap_to_string : 'h -> string) (ret : ('v,'h) t) : string =
  let (_,_,st,_,pc,h), out = ret in "#RETURN:\n" ^
...
let print (to_string : 'v -> string) (heap_to_string : 'h -> string) (ret : ('v,'h) t) : unit =
