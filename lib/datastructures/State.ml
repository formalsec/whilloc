type ('v,'h) t = Program.stmt * (Program.stmt list) * 'v Store.t * 'v Callstack.t * 'v PathCondition.t * 'h

let get_statement (state : ('v,'h) t) : Program.stmt =
  let stmt,_,_,_,_,_=state in stmt

let get_continuation (state : ('v,'h) t) : Program.stmt list =
  let _,cont,_,_,_,_=state in cont

let get_store (state : ('v,'h) t) : 'v Store.t =
  let _,_,st,_,_,_=state in st

let get_callstack (state : ('v,'h) t) : 'v Callstack.t =
  let _,_,_,cs,_,_=state in cs

let get_pathcondition (state : ('v,'h) t) : 'v PathCondition.t =
  let _,_,_,_,pc,_=state in pc

let push_statements (state : ('v,'h) t) (to_add : Program.stmt list) : ('v,'h) t =
  let stmt,stmts,st,cs,pc,heap = state in
  (stmt,to_add@stmts,st,cs,pc,heap)

let to_string (str : 'v -> string) (heap_str : 'h -> string) (state : ('v,'h) t) : string =
  let s,cont,store,cs,pc,heap = state in ">STATE:\n" ^
...
let print (str : 'v -> string) (heap_str : 'h -> string) (state : ('v,'h) t) : unit = 
