...
let step (prog : program) (state : SState.t) : Return.t list = 
  ...
  | Return e ->
      ...
      let cs'   = SCallstack.pop cs in
        (match frame with
        | SCallstack.Intermediate (store',rest,var) ->  let store'' = Hashtbl.copy store' in (* necessary when the function body branches and returns different values *)
...
let rec search (gas : int) (prog : program) (states : SState.t list) (returns : Return.t list) (tree : Program.stmt BinaryTree.t) : Return.t list * Program.stmt BinaryTree.t = 
  ...
  search (gas-1) prog (join_enqueue states_cont states') ( branches_final @ returns ) tree'


let interpret (prog : program) (main_id : string) : Return.t list = 
  let main  = get_function main_id prog in
  let tank  = Parameters.tank in
  let store = SStore.create_empty_store 100 in
