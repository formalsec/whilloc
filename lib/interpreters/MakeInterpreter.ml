module M 
  ...
  let initial_state (program : Program.program) : (state * Program.stmt list) =
    ...
    let state = 
      SState.{ 
        ...
        cs = Callstack.create_callstack } in 
    (state, [ main.body ])
  ...
  let rec step 
    ...
    | FunCall (x,f,es) ->
        ...
        let xvs    = 
          ...
          with _ -> failwith ("TypeError: Argument arity mismatch when calling " ^ f) in
    ...
    | While (e,body) as while_stmt ->
      ...
      let/ b  = Choice.select v in
      if b 
        then return (body::while_stmt::cont)
        else return cont
    ...
    | Assume e ->
      ...
      if b 
        then return cont
        else Choice.return Outcome.AssumeF
    ...
    | Assert e ->
      ...
      if b 
        then return cont
        else Choice.return @@ Outcome.Error Model.empty (* falta colocar o modelo *)
    ...
    | New (x, e) ->
      let f_new (s : state) = 
        ...
        List.map (fun (hp, loc, pc') -> 
          (loc, SState.{ heap = hp; pc = pc'; store = Store.dup s.store; cs = Callstack.dup s.cs })  
        ) lst in 
      ...
      let f_update (s : state) =
        ...
        | Some loc ->
          ...
          if b then
            ...
            List.map (fun (hp, pc') ->
              let store', cs' = if dup then Store.dup s.store, Callstack.dup s.cs else s.store, s.cs in
              ((), SState.{ heap = hp; pc = pc'; store = store'; cs = cs'})
            ) lst
          else failwith "Index out of bounds"
        | None -> failwith "InternalError: array is not defined") in 
  ...
  *)
  ...
  let rec search 
    ...
    if gas=0 
      then Choice.return Outcome.EndGas 
      else (
        match cont with 
        | [] -> assert false 
        | s::cont' ->
