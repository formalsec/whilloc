...
let translate_uop (op : uop) (v : Enc.Expression.t) : Enc.Expression.t =
  ...
  | _ ->
      failwith
        ("TODO: Encoding.encode_unop, missing implementation of "
       ^ string_of_uop op)

let translate_binop (op : bop) (v1 : Enc.Expression.t) (v2 : Enc.Expression.t) :
    Enc.Expression.t =
  match op with
...
let translate_from_encoding (expr : Enc.Symbol.t * Enc.Value.t) :
    string * Value.t =
  match expr with
...
let find_model (exprs : Expression.t list) ?(print_model = false) () =
  ...
  | Some m ->
      let model =
        Some (List.map translate_from_encoding (Enc.Model.get_bindings m))
      in
