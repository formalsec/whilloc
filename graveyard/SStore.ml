type t = (string, Expression.t) Hashtbl.t (*partial function from Var to Expressions*)

let create_empty_store (size : int) : t =
  Hashtbl.create size

let create_store (varvals : (string * Expression.t) list) : t =
  let st = Hashtbl.create (List.length varvals) in
  List.iter (fun (x, v) -> Hashtbl.replace st x v) varvals;
  st

let set (st : t) (var : string) (e : Expression.t) =
...
let get (st : t) (var : string) =
  ...
  | Some v  -> v) (* here, 'v' will always be a Val.t *)
